...
<body>
	...
	<ul>
		...
		<?php } ?>
		<?php if ($this->session->userdata('user') == 'karyawan') { ?>
			<li class="lisub active" onclick="window.location.href='<?php echo site_url('tarik_tunai') ?>';"><i class="fas fa-money-bill"></i>Ambil Tabungan</li>
	...
	<div class="content">
		<div class="col-6 left">
			<h1>Ambil Tabungan</h1>
			<?php if (!isset($_GET['p']) || $_GET['p'] == 'cari') { ?>
				<button class="sub" onclick="window.location.href='<?php echo site_url('tarik_tunai') ?>?p=tambah';">Tambah</button>
			<?php } ?>
		</div>
			...
			<div class="box shadow">
				...
				<table>
					<?php if (!isset($_GET['p']) || $_GET['p'] == 'cari') { ?>
						...
						<tbody>
							<?php
							$this->db->order_by('ambil_tabungan.id_ambil desc');
							$this->db->select('*');
							$this->db->from('ambil_tabungan');
							$this->db->join('anggota', 'anggota.id_anggota=ambil_tabungan.id_anggota');
							...
							?>
								<tr>
									...
									<td align="center">Rp.<?php echo number_format($row->jumlah) ?></td>
								</tr>
							<?php } ?>
						</tbody>
					<?php } else { ?>
						<tbody class="nohover">
							...
							<tr>
								...
								<td>
									<select id="anggota">
										<option selected value="0"></option>
										<?php
										$this->db->order_by('anggota.nama asc');
										$this->db->select('*,anggota.id_anggota AS id_a');
										$this->db->from('tabungan');
										$this->db->join('anggota', 'anggota.id_anggota=tabungan.id_anggota');
										$this->db->where('tabungan.jumlah >=', '500000');
										$sql = $this->db->get();

										foreach ($sql->result() as $row) {
										?>
											<option value="<?php echo $row->id_a ?>"><?php echo $row->nama; ?> | Saldo : Rp.<?php echo number_format($row->jumlah) ?></option>
										<?php } ?>
									</select>
								</td>
							</tr>
							...
							<h5 id="limit" hidden style="color: red">*Batas pengambilan uang adalah setengah dari saldo</h5>
							<tr>
								<td></td>
								<td></td>
								<td>
									<button id="simpan" class="sub">Proses</button>
									<button onclick="window.location.href='<?php echo site_url('tarik_tunai') ?>';" class="sub cancel">Batal</button>
								</td>
							</tr>
						</tbody>
					<?php } ?>
	...
	<script>
		...
		$('#bcari').click(function() {
			...
			$.ajax({
				type: 'post',
				url: '<?php echo base_url() ?>tarik_tunai/cari',
				dataType: 'json',
				data: {
					nama: nama
				},
				success: function(result) {
					window.location.href = '<?php echo base_url() ?>tarik_tunai?p=cari';
				}
			});
		});
		...
		$('#proses').click(function() {
			...
			$.ajax({
				type: 'post',
				url: '<?php echo base_url() ?>tarik_tunai/proses',
				dataType: 'json',
				data: {
					tgl: tgl,
					bln: bln,
